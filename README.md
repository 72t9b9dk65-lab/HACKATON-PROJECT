...
**Distribute transactions to donators**, beside **All care transactions**, rebalances the complete transaction list across the donor portfolios. Existing cost assignments are released and the same costs are assigned again in one atomic update, capped by each donor's available funding after that release. Receipt totals, total Used, photos and original dates stay unchanged; individual Pending/Used balances and care shares update. Repeating the action with unchanged donations produces the same allocation, never another charge. The override is stored separately from imported source expenses and original receipt records.

1. Attach an invoice or receipt (PDF/JPG/PNG/WebP, up to 1 MB). Enter the supplier, reference, purchase date, product descriptions, categories and line totals including VAT. Document extraction is manual; no OCR, backend, sign-in or external connection is implemented.
2. Review and record the automatic allocation. The full receipt is divided equally across funded portfolios in integer öre. Wallets are capped at their available balance; remaining costs are redistributed. Rounding is conserved across all invoice lines. Insufficient total funds, duplicate supplier/reference pairs, invalid lines or failed storage writes leave the complete receipt unrecorded and all balances unchanged.
