...
The homepage centers on a personal shelter profile and activity scene. A left column contains the donated total split into Pending and Used, donation choices, and dated care transactions. Clicking the total opens a spending breakdown above the shelter; the Sweden map and real photo journey remain below the workspace.
...
- The shelter scene has a large communal kennel above one row of park walks, meals, recovery, veterinarian, and playtime stations. A simple repeating activity clock moves dogs between destinations, switches the background between day and night, and returns sleeping dogs to the kennel with floating Z letters. **Day view / Night view** allows a direct preview. The clock pauses for open profiles, keyboard focus, hidden tabs, and reduced-motion preferences. This daily routine never advances the donation timeline or ledger; medical visits appear only for companions with care scheduled on the selected forecast day. Compact screens can pan the scene, and larger populations scroll vertically.
...
- For a one-time gift, **Donate** records the amount and simulates one first-day care unit per matched real dog. That expense enters Used; the rest stays Pending. A sub-unit contribution stays entirely Pending and creates no permanent companion. Historical gift allocations remain unchanged and are not treated as proof of spending. Monthly forecasts and scene animations never create expenses. The total always equals Pending plus Used in integer öre.
- A fresh session retains the labeled 500 SEK gift example. Two explicit sample expenses illustrate 100 SEK for Åke’s food and 120 SEK for Ove’s care: 220 SEK Used and 280 SEK Pending. Prior user gifts keep their original amounts, dates, and recipient allocations, and remain Pending unless an expense exists. The map displays recorded demo spending and does not claim that every dog lives at its listed shelter continuously.
- Clicking the balance reveals vertical columns for every goods/services category, including zero-spend categories. Switch to **Dogs in your shelter** for exact spending per dog, with real rounded-square portraits and pixel badges. Both views reconcile to Used.
- The transaction list below donation choices shows an expense icon, recipient, amount, and date/time in Europe/Stockholm. Clicking a transaction or a dog spending column replays its saved care category: the recipient moves from the kennel to the relevant station. The replay uses the recorded timestamp, highlights the recipient, respects pause/reduced motion, and never changes balances. **Back to routine** resumes the ordinary scene. These are illustrated demo replays, not organization-posted evidence.
